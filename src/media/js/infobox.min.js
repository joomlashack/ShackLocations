...
InfoBox.prototype=new google.maps.OverlayView();InfoBox.prototype.createInfoBoxDiv_=function(){let i,events,bw,me=this;let cancelHandler=function(event){event.cancelBubble=true;if(event.stopPropagation){event.stopPropagation();}};let ignoreHandler=function(event){event.preventDefault();if(!me.enableEventPropagation_){cancelHandler(event);}};if(!this.div_){this.div_=document.createElement('div');this.setBoxStyle_();if(typeof this.content_.nodeType==='undefined'){this.div_.innerHTML=this.content_;}else{this.div_.appendChild(this.content_);}
this.div_.prepend(this.getCloseBoxImg_());this.getPanes()[this.pane_].appendChild(this.div_);this.addClickHandler_();if(this.div_.style.width){this.fixedWidthSet_=true;}else{if(this.maxWidth_!==0&&this.div_.offsetWidth>this.maxWidth_){this.div_.style.width=this.maxWidth_;this.div_.style.overflow='auto';this.fixedWidthSet_=true;}else{bw=this.getBoxWidths_();this.div_.style.width=(this.div_.offsetWidth-bw.left-bw.right)+'px';this.fixedWidthSet_=false;}}
this.panBox_(this.disableAutoPan_);if(!this.enableEventPropagation_){this.eventListeners_=[];events=['mousedown','mouseover','mouseout','mouseup','click','dblclick','touchstart','touchend','touchmove'];for(i=0;i<events.length;i++){this.eventListeners_.push(google.maps.event.addDomListener(this.div_,events[i],cancelHandler));}
this.eventListeners_.push(google.maps.event.addDomListener(this.div_,'mouseover',function(){this.style.cursor='default';}));}
...
return img;};InfoBox.prototype.addClickHandler_=function(){if(this.closeBoxURL_===''){this.closeListener_=null;}else{let closeBox=this.div_.firstChild;this.closeListener_=google.maps.event.addDomListener(closeBox,'click',this.getCloseClickHandler_());}};InfoBox.prototype.getCloseClickHandler_=function(){let me=this;return function(e){e.cancelBubble=true;if(e.stopPropagation){e.stopPropagation();}
google.maps.event.trigger(me,'closeclick');me.close();};};InfoBox.prototype.panBox_=function(disablePan){let map,xOffset=0,yOffset=0;if(!disablePan){map=this.getMap();if(map instanceof google.maps.Map){if(!map.getBounds().contains(this.position_)){map.setCenter(this.position_);}
...
if(typeof content.nodeType==='undefined'){this.div_.innerHTML=content;}else{this.div_.appendChild(content);}
this.div_.prepend(this.getCloseBoxImg_());if(!this.fixedWidthSet_){this.div_.style.width=this.div_.offsetWidth+'px';if(typeof content.nodeType==='undefined'){this.div_.innerHTML=content;}else{this.div_.appendChild(content);}
...
if(this.eventListeners_){for(let i=0;i<this.eventListeners_.length;i++){google.maps.event.removeListener(this.eventListeners_[i]);}
