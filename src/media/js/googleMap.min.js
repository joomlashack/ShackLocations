...
setSearch();updateActiveCount();if(!options.show.markers){setTimeout(function(){$(options.selector.mainToggle).trigger('click');},100);}};let updateDisplay=function(delay){updateList(delay);};let initMap=function(){let mapProperties=options.mapProperties,mapCenter=mapProperties.center;mapProperties.mapTypeId=google.maps.MapTypeId[mapProperties.mapTypeId]||null;if(mapCenter.lat&&mapCenter.lng){mapProperties.center=new google.maps.LatLng(mapCenter.lat,mapCenter.lng);}else{mapProperties.center=null;}
...
mappedMarkers.push(marker.id);markerSets[marker.typeId].push(marker.id);});if(options.show.clusters){clusterManager=new MarkerClusterer(map,clusterMarkers,options.clusterOptions);}};let updateActiveCount=function(){if(options.show.legend){let displayText='',displayArguments=[],status='',activeCount=0,$noLocations=$('.nolocations');markers.forEach(function(marker,id){if(marker.status>0){activeCount+=1;status+=marker.status;}});if(options.show.listTab){if(activeCount===0){if($noLocations.length===0){setTimeout(function(){$noLocations=$('<div class="nolocations"/>').html(Joomla.Text._('COM_FOCALPOINT_NO_LOCATION_TYPES_SELECTED')).appendTo($listHolder);},100);}}else{$noLocations.remove();}}
if(search.text!==''){displayText='COM_FOCALPOINT_SEARCH_WITHIN';displayArguments=[activeCount,options.search.radius,search.text];}else{displayText='COM_FOCALPOINT_SEARCH_SHOWING';displayArguments=[activeCount]}
...
bounds.extend(marker.getPosition());}});if(bounds!==null){map.fitBounds(bounds);}else{if(options.mapProperties.center){map.panTo(options.mapProperties.center);}
map.setZoom(options.mapProperties.zoom);}}
if(options.show.clusters){clusterMarkers=[];markers.forEach(function(marker){if(marker.status>0){clusterMarkers.push(marker);}});clusterManager.clearMarkers();clusterManager=new MarkerClusterer(map,clusterMarkers,options.clusterOptions);}
updateActiveCount();updateList(150);};let resetMap=function(evt){allowScrollTo=false;search.text='';$(options.selector.searchAddress).val(search.text);markers.forEach(function(marker,id){if(marker.status<-999){marker.status+=5000;marker.setMap(map);$('.fp_list_marker'+id).fadeIn(100);}});$(options.selector.mainToggle).each(function(){let $toggle=$(this),$markerToggles=$(options.selector.legendToggles);if(options.show.markers){$toggle.data('togglestate','off').html(Joomla.Text._('COM_FOCALPOINT_BUTTON_HIDE_ALL'));$markerToggles.each(function(){let $markerToggle=$(this);if($markerToggle.hasClass('active')){$markerToggle.trigger('click');}
$markerToggle.trigger('click');});}else{$toggle.data('togglestate','on').html(Joomla.Text._('COM_FOCALPOINT_BUTTON_SHOW_ALL'));$markerToggles.each(function(){let $markerToggle=$(this);if($markerToggle.hasClass('active')){$markerToggle.trigger('click');}});}});allowScrollTo=true;map.panTo(options.mapProperties.center);map.setZoom(options.mapProperties.zoom);options.markerData.forEach(function(marker,id){if(marker.infoBox.event==='always'&&markers[marker.id].getVisible()){markerInfoBox[marker.id].open(map,markers[marker.id]);}});};let toggleMarkers=function(evt){allowScrollTo=false;let $this=$(this),$toggles=$(options.selector.legendToggles);if($this.data('togglestate')==='on'){$this.data('togglestate','off').html(Joomla.Text._('COM_FOCALPOINT_BUTTON_HIDE_ALL'));$toggles.each(function(){if(!$(this).hasClass('active')){$(this).trigger('click');}});}else{$this.data('togglestate','on').html(Joomla.Text._('COM_FOCALPOINT_BUTTON_SHOW_ALL'));$toggles.each(function(){if($(this).hasClass('active')){$(this).trigger('click');}});}
...
updateActiveCount();allowScrollTo=true;});if(options.show.clusters){clusterMarkers=[];markers.forEach(function(marker){if(marker.status>0){clusterMarkers.push(marker);}});clusterManager.clearMarkers();clusterManager=new MarkerClusterer(map,clusterMarkers,options.clusterOptions);}
map.setCenter(location);map.setZoom(options.search.zoom);updateList(500);}else{alert(Joomla.Text._('COM_FOCALPOINT_ERROR_GEOCODE').replace('%s',status));}});}else{alert(Joomla.Text._('COM_FOCALPOINT_SEARCH_ADDRESS_REQUIRED'));}});}}
let updateList=function(delay){if(options.show.listTab){let update=function(){let locationListHeight=$listHolder.outerHeight();if(locationListHeight>0){$listHolder.parent().css('min-height',locationListHeight);}};if(delay){setTimeout(function(){update();},delay);}else{update();}}};return{init:init,update:updateDisplay,};};});