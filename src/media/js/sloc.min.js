(function($){$.sloc=$.extend(true,{map:{instance:{}}},$.sloc);$.extend(true,$.sloc,{loadScript:function(uri){return new Promise((resolve,reject)=>{let tag=document.createElement('script');tag.src=uri;tag.async=true;tag.onload=resolve;tag.onerror=reject;let scriptTag=document.getElementsByTagName('script')[0];scriptTag.parentNode.insertBefore(tag,scriptTag);});},sprintf:function(string,replacements){let result=string.toString().replace(/%%/g,'%');if(arguments.length>2){let trimmed=Object.keys(arguments).map((key)=>arguments[key]);replacements=trimmed.slice(1);}else if(replacements.constructor===String){replacements=[replacements]}
